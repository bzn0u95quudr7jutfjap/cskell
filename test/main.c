...
int test_tokenization_m(char *name, Stack_String (*function)(String *), String *input, Stack_String output_o) {
  Stack_String *output = &output_o;
  printf("%6s :: %s\n", "INIZIO", name);
  ...
  for (int i = 0; i < a.size && i < output->size; i++) {
    errcode += !equals_string(&(a.data[i]), &(output->data[i]));
  }
  // push_stack_test(&g_tests, (testdata){.name = name, .success = errcode == 0});
  //printf("%6s :: [%3s] :: %10s :: ", "STATUS", status(errcode == 0), "Atteso");
  print_stack_string(output);
  printf("\n");
  //printf("%6s :: [%3s] :: %10s :: ", "STATUS", status(errcode == 0), "Ottenuto");
  print_stack_string(&a);
  printf("\n");
  printf("%6s :: %s\n", "FINE", name);
  free_String(input);
    ...
    push_stack_test(&g_tests, (testdata){                                                                              \
                                  ...
                                  .tokenizer =                                                                         \
                                      {                                                                                \
                                          ...
                                          .expect = ss_ca(sizeof(o) / sizeof(*o), o),                                  \
                                      },                                                                               \
                              });                                                                                      \
  } while (0)
...
int main(int argc, char *argv[]) {
  ...
  push_test_tokenizer("token_commenti_multilinea_senza_terminatore", "int a; /* int a = 12;\nint b;",
                      {"int", "a", ";", "/* int a = 12;\nint b;"});
  // test per le macro
  push_test_tokenizer("macro_semplice", "#define a 0", {"#", "define", "a", "0", "\n"});
  push_test_tokenizer("macro_semplice_con_concatenazione", "#define a a##a",
                      {"#", "define", "a", "a", "#", "#", "a", "\n"});
  push_test_tokenizer("macro_semplice_con_concatenazione_con_nomi_strani", "#define a a_##a",
                      {"#", "define", "a", "a_", "#", "#", "a", "\n"});
  push_test_tokenizer("macro_funzionale_semplice", "#define a() a", {"#", "define", "a", "(", ")", "a", "\n"});
  push_test_tokenizer("macro_funzionale_semplice", "#define count(a) (sizeof(a)/sizeof(*a))",
